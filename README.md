...
- каталог продуктов загружается backend-ом автоматически из PDF-таблиц при старте;
...
После первого запуска backend:

1. извлечёт продукты из PDF-файлов в папке `pdf продуктов`;
2. очистит дубли и усреднит диапазоны вида `15-20`;
3. добавит каталог в таблицу `nutrition_products`, не создавая повторов;
4. позволит пользователю добавлять свои продукты прямо из интерфейса.
...
Пересобрать JSON-каталог из PDF вручную:

```bash
python3 database/scripts/build_nutrition_seed.py
```
